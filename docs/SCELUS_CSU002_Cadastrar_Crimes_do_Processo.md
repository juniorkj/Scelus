...
| Editar/Visualizar (`/crimes/:id`) | ✅ Implementado (14/07/2026) | O antigo formulário plano (`consultar-crimes-manter`) foi **aposentado** — `/crimes/:id` agora abre o mesmo wizard por passos do cadastro novo, pré-carregado via `GET /api/crimes/{id}/completo`. Segue a convenção existente do `TjCrudBaseComponent`: `?_readonly=true` na mesma rota alterna para modo somente leitura (fieldset desabilitado), sem rota própria de edição. |
| Atualização (`PUT /api/crimes/{id}/completo`) | ✅ Implementado | Update **cirúrgico por tabela** (não é "apagar tudo e recriar"): perfil da litigância via `pkg_litigancia.fn_litigancia_upd`; associações (ocupação/deficiência/droga/escuta judicial), benefícios, configuração familiar, comunicantes e consequências são substituídos por bloco (exclui os existentes daquele bloco e reinsere os atuais) — preservando os IDs estáveis (litigância, vítima, acusado, fato ocorrido, processo de crime). Crimes cometidos removidos da tela **não** são excluídos automaticamente (evita órfãos por FK); apenas novos/atualizados são gravados. |
| MPU no fluxo por passos | ✅ Portado | A gestão de vínculo de MPU (CSU008 — vincular/remover) que existia só no formulário antigo foi portada para o passo "Fato Ocorrido" do wizard, visível apenas quando há um `idFatoOcorrido` (ou seja, em edição/visualização). |
