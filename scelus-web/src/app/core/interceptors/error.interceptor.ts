...
export class ErrorInterceptor implements HttpInterceptor {
    ...
    return next.handle(req).pipe(
      catchError((error: HttpErrorResponse) => {
        ...
        const title =
          ...
          this.resolveTitle(error.status);
  ...
  private parseBody(error: HttpErrorResponse): Record<string, unknown> {
    ...
    try {
      return JSON.parse(error.error as string);
    } catch {
      return {};
    }
  }
  ...
  private resolveMessage(
    ...
    const bodyMessage = (body['message'] ?? body['detail']) as
      string | undefined;
    ...
    switch (status) {
      ...
      case 422:
        return 'A operação não pôde ser concluída. Verifique as regras de negócio.';
      case 400:
